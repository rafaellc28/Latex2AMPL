...
class CodeGenerator:
    ...
    # Get the AMPL code for a given relational expression
    ...
    def _posModel(self):
        res = ""
        #res += "\n\n"+self._declareDataSetsAndParams()
        #res += "\nsolve;"
    ...
    def generateCode_NumericExpressionWithFunction(self, node):
        ...
        if node.numericExpression1 != None:
            #print("2", node.numericExpression1, str(node.numericExpression1))
    ...
    def generateCode_IteratedSetExpression(self, node):
        ...
        if node.supExpression:
            res += " {" + node.indexingExpression.generateCode(self) + "..(" + node.supExpression.generateCode(self) + ")} "
        else:
